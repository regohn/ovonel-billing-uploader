"""
Ovonel Billing Audit Pipeline — Render API (main.py) v2.3
==========================================================
Changes in v2.2:
...
app = FastAPI(title="Ovonel Billing Extraction API", version="2.3.0")
...
VISION_PROMPT = """You are parsing a Philippine SERVICE INVOICE. Extract exactly these 12 fields.
...
date:
  - The PAYMENT DATE — the actual transaction date of the invoice
  - Location: labelled "Payment Date:" on the invoice, usually in the upper right area
  - Format as MM/DD/YYYY (e.g. "02/09/2026")
  - CRITICAL: Do NOT use the BIR issuance date, the "Date Issued" printed near the
    bottom left (near the printer/BIR authority information), or the "Valid Until" date.
    Those are BIR registration dates — NOT the invoice payment date.
  - ONLY extract the date next to "Payment Date:" label
  - If "Payment Date:" is blank or not present, return null
...
date: The PAYMENT DATE labelled "Payment Date:" — the actual transaction date (MM/DD/YYYY).
      Do NOT use "Date Issued", "Valid Until", or any BIR registration date at the bottom.
      Return null if "Payment Date:" is blank.
...
@app.get("/")
def health():
    return {"status": "ok", "service": "Ovonel Billing Extraction API v2.3"}
